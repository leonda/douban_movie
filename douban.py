...
def bug_catch(file_name, url):
    ...
    for j in range(0, 7):
        ...
        for i in content:
            ...
            dic['area'] = dic['area'].strip()
            dic['time'] = dic['time'].strip()
            dic['total_price'] = dic['total_price'].strip()
            csvwriter.writerow(dic.values())

            sql = '''INSERT INTO %s (film_name,rating,kind,area,time,total_price) 
                    values('%s','%s','%s','%s','%s','%s')''' % (table_name, dic['film_name'], dic['rating'],
                                                                dic['kind'], dic['area'], dic['time'], dic['total_price'])
            c.execute(sql)
            conn.commit()
        ...
        resp.close()
    f.close()
    print(file_name, "年数据手刃完毕,已导出至", file_name,
          ".csv和数据库film_info.db的表", table_name)
...
def main():
    ...
    resp_content = requests.get(
        "https://www.douban.com/doulist/135651096/", headers=headers).text
    content = obj_menu.finditer(resp_content)
    for it in content:
        print("读取到数据"+it.group("year")+" : " +
              it.group("domain_pre") + it.group("domain_suffix"))
        bug_catch(it.group("year"), it.group(
            "domain_pre") + it.group("domain_suffix"))
